...
public class NotificationEventConsumer {
    ...
    public void onNotificationCreate(NotificationCreateEvent event, Message message, Channel channel)
            throws IOException {
        ...
        try {
            ...
            if (eventKey != null && !idempotencyMarked) {
                log.info("通知创建事件已消费，跳过重复消息，eventId={}", event.getEventId());
            ...
            log.error("通知创建事件消费失败，userId={}, businessType={}, businessId={}",
                    event.getUserId(), event.getBusinessType(), event.getBusinessId(), exception);
            mqFailureRecorder.recordConsumerFailure(MqFailureRecorder.NOTIFICATION_CREATE, event, exception);
    ...
    private void releaseIdempotencyMarker(String eventKey) {
        try {
            ...
            log.warn("通知创建事件幂等标记释放失败，eventKey={}", eventKey, exception);
        }
    }
}
