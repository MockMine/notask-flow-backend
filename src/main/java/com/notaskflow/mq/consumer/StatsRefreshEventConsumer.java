...
public class StatsRefreshEventConsumer {
    ...
    public void onStatsRefreshRequested(StatsRefreshRequestedEvent event, Message message, Channel channel)
            throws IOException {
        ...
        try {
            ...
            if (eventKey != null && !idempotencyMarked) {
                log.info("统计刷新事件已消费，跳过重复消息，eventId={}", event.getEventId());
            ...
            log.error("统计刷新事件消费失败，spaceId={}, scope={}",
                    event.getSpaceId(), event.getScope(), exception);
            mqFailureRecorder.recordConsumerFailure(MqFailureRecorder.STATS_REFRESH_REQUESTED, event, exception);
            channel.basicNack(deliveryTag, false, false);
        }
    }

    private void refreshStats(StatsRefreshRequestedEvent event) {
        if (event.getSpaceId() == null) {
            return;
        }
        if (StatsRefreshScope.FILE.equals(event.getScope())) {
            fileStatsCacheService.refresh(event.getSpaceId());
        }
    }
    ...
    private void releaseIdempotencyMarker(String eventKey) {
        try {
            ...
            log.warn("统计刷新事件幂等标记释放失败，eventKey={}", eventKey, exception);
        }
    }
}
